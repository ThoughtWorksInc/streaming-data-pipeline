...
nohup java -jar /apps/CitibikeApiProducer/build/libs/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=station-information --kafka.brokers=localhost:9092 1>/tmp/apps/station-information.log 2>/tmp/apps/station-information.error.log &

nohup java -jar /apps/CitibikeApiProducer/build/libs/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=station-san-francisco --kafka.brokers=localhost:9092 1>/tmp/apps/station-san-francisco.log 2>/tmp/apps/station-san-francisco.error.log &

nohup java -jar /apps/CitibikeApiProducer/build/libs/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=station-status --kafka.brokers=localhost:9092 1>/tmp/apps/station-status.log 2>/tmp/apps/station-status.error.log &
 
 echo "====Producers Deployed===="

echo "====Deploy Raw Data Saver===="

nohup spark-submit --master spark://$SPARK_HOST_NAME:7077 --class com.free2wheelers.apps.StationLocationApp --name StationStatusSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 1024M --conf spark.executor.memory=2g --conf spark.cores.max=1 /apps/RawDataSaver/target/scala-2.11/free2wheelers-raw-data-saver_2.11-0.0.1.jar localhost:2181 "/free2wheelers/stationStatus" 1>/tmp/apps/raw-station-status-data-saver.log 2>/tmp/apps/raw-station-status-data-saver.error.log &
