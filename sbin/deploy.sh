...
if [[ -z "${TRAINING_COHORT}" ]]; then
    echo "No training cohort configured, consider setting the TRAINING_COHORT environment variable (in circle ci also) ."
    exit 1
fi
...
DEPLOYMENT_IDENTIFIER="data-eng-${TRAINING_COHORT}"
...
scp ./zookeeper/seed.sh kafka.${DEPLOYMENT_IDENTIFIER}.training:/tmp/zookeeper-seed.sh
ssh kafka.${DEPLOYMENT_IDENTIFIER}.training '
set -e
export hdfs_server="emr-master.${DEPLOYMENT_IDENTIFIER}.training:8020"
export kafka_server="kafka.${DEPLOYMENT_IDENTIFIER}.training:9092"
...
scp CitibikeApiProducer/build/libs/free2wheelers-citibike-apis-producer0.1.0.jar ingester.${DEPLOYMENT_IDENTIFIER}.training:/tmp/
echo "====Jar copied to ingester server===="

ssh ingester.${DEPLOYMENT_IDENTIFIER}.training '
set -e
...
nohup java -jar /tmp/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_information} --kafka.brokers=kafka.${DEPLOYMENT_IDENTIFIER}.training:9092 1>/tmp/${station_information}.log 2>/tmp/${station_information}.error.log &
nohup java -jar /tmp/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_san_francisco} --producer.topic=station_data_sf --kafka.brokers=kafka.${DEPLOYMENT_IDENTIFIER}.training:9092 1>/tmp/${station_san_francisco}.log 2>/tmp/${station_san_francisco}.error.log &
nohup java -jar /tmp/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_status} --kafka.brokers=kafka.${DEPLOYMENT_IDENTIFIER}.training:9092 1>/tmp/${station_status}.log 2>/tmp/${station_status}.error.log &
...
scp ./hdfs/seed.sh emr-master.${DEPLOYMENT_IDENTIFIER}.training:/tmp/hdfs-seed.sh

ssh emr-master.${DEPLOYMENT_IDENTIFIER}.training '
set -e
export hdfs_server="emr-master.${DEPLOYMENT_IDENTIFIER}.training:8020"
...
scp RawDataSaver/target/scala-2.11/free2wheelers-raw-data-saver_2.11-0.0.1.jar emr-master.${DEPLOYMENT_IDENTIFIER}.training:/tmp/
echo "====Raw Data Saver Jar Copied to EMR===="

scp sbin/go.sh emr-master.${DEPLOYMENT_IDENTIFIER}.training:/tmp/go.sh
...
scp StationConsumer/target/scala-2.11/free2wheelers-station-consumer_2.11-0.0.1.jar emr-master.${DEPLOYMENT_IDENTIFIER}.training:/tmp/

scp StationTransformerNYC/target/scala-2.11/free2wheelers-station-transformer-nyc_2.11-0.0.1.jar emr-master.${DEPLOYMENT_IDENTIFIER}.training:/tmp/
