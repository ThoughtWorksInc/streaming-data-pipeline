...
nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationLocationApp --name StationDataFranceSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --driver-memory 500M --conf spark.executor.memory=1g --conf spark.cores.max=1 /tmp/free2wheelers-raw-data-saver_2.11-0.0.1.jar kafka.twdu1.training:2181 "/free2wheelers/stationDataFrance" 1>/tmp/raw-station-data-france-saver.log 2>/tmp/raw-station-data-france-saver.error.log &
