...
nohup java -jar /usr/lib/citibike-apps/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_san_francisco} --producer.topic=station_data_sf --kafka.brokers=kafka.twdu1-uat.training:9092 1>/tmp/${station_san_francisco}.log 2>/tmp/${station_san_francisco}.error.log &
nohup java -jar /usr/lib/citibike-apps/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_nyc} --producer.topic=station_data_nyc_v2 --kafka.brokers=kafka.twdu1-uat.training:9092 1>/tmp/${station_nyc}.log 2>/tmp/${station_nyc}.error.log &
nohup java -jar /usr/lib/citibike-apps/free2wheelers-citibike-apis-producer0.1.0.jar --spring.profiles.active=${station_marseille} --kafka.brokers=kafka.twdu1-uat.training:9092 1>/tmp/${station_marseille}.log 2>/tmp/${station_marseille}.error.log &
...
nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationLocationApp --queue streaming --name StationInformationSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --conf spark.executor.memory=512m --conf spark.cores.max=1 /usr/lib/citibike-apps/free2wheelers-raw-data-saver_2.11-0.0.1.jar ${zookeeper_connection_string} "/free2wheelers/stationInformation" 1>/tmp/raw-station-information-data-saver.log 2>/tmp/raw-station-information-data-saver.error.log &

# Sleep between two spark-submit executions to prevent the following error: org.xml.sax.SAXParseException; systemId: file:/home/hadoop/.ivy2/cache/org.apache.spark-spark-submit-parent-default.xml; lineNumber: 1; columnNumber: 1; Premature end of file
# This workaround can be removed when Spark is upgraded to 2.3.1+
sleep 1m

nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationLocationApp --queue streaming --name StationDataSFSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --conf spark.executor.memory=512m --conf spark.cores.max=1 /usr/lib/citibike-apps/free2wheelers-raw-data-saver_2.11-0.0.1.jar ${zookeeper_connection_string} "/free2wheelers/stationDataSF" 1>/tmp/raw-station-data-sf-saver.log 2>/tmp/raw-station-data-sf-saver.error.log &

# Sleep between two spark-submit executions to prevent the following error: org.xml.sax.SAXParseException; systemId: file:/home/hadoop/.ivy2/cache/org.apache.spark-spark-submit-parent-default.xml; lineNumber: 1; columnNumber: 1; Premature end of file
# This workaround can be removed when Spark is upgraded to 2.3.1+
sleep 1m

nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationLocationApp --queue streaming --name StationDataNYCSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --conf spark.executor.memory=512m --conf spark.cores.max=1 /usr/lib/citibike-apps/free2wheelers-raw-data-saver_2.11-0.0.1.jar ${zookeeper_connection_string} "/free2wheelers/stationDataNYCV2" 1>/tmp/raw-station-data-nyc-saver.log 2>/tmp/raw-station-data-nyc-saver.error.log &

# Sleep between two spark-submit executions to prevent the following error: org.xml.sax.SAXParseException; systemId: file:/home/hadoop/.ivy2/cache/org.apache.spark-spark-submit-parent-default.xml; lineNumber: 1; columnNumber: 1; Premature end of file
# This workaround can be removed when Spark is upgraded to 2.3.1+
sleep 1m

nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationLocationApp --queue streaming --name StationDataMarseilleSaverApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0 --conf spark.executor.memory=512m --conf spark.cores.max=1 /usr/lib/citibike-apps/free2wheelers-raw-data-saver_2.11-0.0.1.jar ${zookeeper_connection_string} "/free2wheelers/stationDataMarseille" 1>/tmp/raw-station-data-marseille-saver.log 2>/tmp/raw-station-data-marseille-saver.error.log &
...
nohup spark-submit --master yarn --deploy-mode cluster --class com.free2wheelers.apps.StationApp --queue streaming --name StationApp --packages org.apache.spark:spark-sql-kafka-0-10_2.11:2.3.0  --conf spark.executor.memory=512m --conf spark.cores.max=1 /usr/lib/citibike-apps/free2wheelers-station-consumer_2.11-0.0.1.jar kafka.twdu1-uat.training:2181 1>/tmp/station-consumer.log 2>/tmp/station-consumer.error.log &
